			...
			.extendModule(
					"mor.ticket.util",
					{
								...
								onSuccess : function(response) {
									...
									if (invocationResult.authRequired
										...
										if (window.ticketStorage
												.getItem("autologin") == "true" && (!latestTime || (latestTime - Date.now() < 7*24*3600*1000))) {
									...
									if (busy.isVisible()) {
										// console.log('Hide busy indicator in
										// commonOptions onSuccess if
										// possible');
										busy.hide();
									}
								},
							...
							function onCheckcodeSuccess(checkcode) {
								...
								function invokeProcedureByGet(url,
									...
									if (invocationData.parameters) {
										requestOptions.parameters.parameters = WLJSX.Object
												.toJSON(invocationData.parameters);
									}
									if (invocationData.parameters) {
										requestOptions.parameters.parameters = WLJSX.Object
												.toJSON(invocationData.parameters);
						...
						prepareRequestCommonParameters : function(
							...
							var time_str = serverTime.formatWithTimezone(
									"yyyyMMddhhmmss", 8);
							var check_code = hex_md5('123456' + time_str
									+ common['baseDTO.device_no']);
							var parameters = {
								"baseDTO.os_type" : common['baseDTO.os_type'],
								"baseDTO.device_no" : common['baseDTO.device_no'],
								"baseDTO.mobile_no" : common['baseDTO.mobile_no'],
