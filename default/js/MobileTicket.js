...
function backFunc() {
	...
	WL.SimpleDialog.show("温馨提示", "是否确认退出应用?", [ {
		text : "取消",
		handler : function() {
		}
	}, {
...
function restoreUserNameAndPass() {
	...
	if(latestTime && latestTime - Date.now() >= 7*24*3600*1000){
		return;
	}
...
function requestSucceeded(result) {
	...
	for ( var i = 0; i < reservePeriodList.length; i++) {
		...
		if (ticket_type == "1" || ticket_type == "2" || ticket_type == "3") {// 只获取到成人（ticket_type=1）的时间
			var util = mor.ticket.util;
	...
	if (serverCache.station_version_no != station_version) {
		...
		if (window.ticketStorage.getItem("stations") != null) {
	...
	if (serverCache.city_version_no != city_version_no) {
		mor.ticket.cache["needSync"] = "Y";
		mor.ticket.cache["syncList"] += "|city";
		mor.ticket.cache["syncVersionList"] += "|" + city_version_no;
	} else {
		// match, load data from local storage
		if (window.ticketStorage.getItem("city") != null) {
	...
	if (serverCache.university_version_no != university_version_no) {
		mor.ticket.cache["needSync"] = "Y";
		mor.ticket.cache["syncList"] += "|university";
		mor.ticket.cache["syncVersionList"] += "|" + university_version_no;
	} else {
		// match, load data from local storage
		if (window.ticketStorage.getItem("university") != null) {
...
function registerAutoLoginHandler() {

	var autologin = window.ticketStorage.getItem("autologin");

	if (autologin == 'true') {

		AutoRestoreUserNameAndPass();
	}
	/*
...
function AutoSendLoginRequest() {
	...
	var options = {
		onSuccess : AutoRequestSucceeded,
		onFailure: util.creatCommonRequestFailureHandler()
	};
...
function AutoRequestSucceeded(result) {
...
function AutoRestoreUserNameAndPass() {

	var user = mor.ticket.loginUser;
	if (user.isKeepUserPW === "true" || user.isKeepUserPW == true) {
		user.username = window.ticketStorage.getItem("username");

		WL.EncryptedCache.open("wlkey", true, AutoOnOpenOk, onOpenError);
	}

}

function AutoOnOpenOk() {
