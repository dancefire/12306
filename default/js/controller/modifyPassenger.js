...
(function() {
	...
	jq("#modifyPassengerView").live("pageinit", function() {
		...
		jq("#modify_PassengerCountry").bind("tap",function(){
			jq.mobile.changePage("selectCountry.html");
			return false;
		});
		
		jq("#modify_pStudentProvince").bind("tap",function(){
			jq.mobile.changePage("selectProvince.html");
			return false;
		});
		
		jq("#modify_pStudentSchool").bind("tap",function(){
		...
		jq("#modify_pPreferenceFromStation").bind("tap",function(){
			mor.ticket.viewControl.isCityGo = true;
			jq.mobile.changePage("selectCity.html");
			return false;
		});
		
		jq("#modify_pPreferenceToStation").bind("tap",function(){
			jq.mobile.changePage("selectCity.html");
			return false;
		});		
		
		jq("#modifyPassengerBtn").bind("tap",function(){
		...
		jq("#deletePassengerBtn").bind("tap",deletePassenger);	
		...
		jq("#modify_PassengerName").change(function () { 
			...
			if(!checkFormUtil.checkChar(jq("#modify_PassengerName").val())){
	...
	function idTypeControl(){
		if(mor.ticket.loginUser.id_type === "1" || mor.ticket.loginUser.id_type === "2"){
			jq("#modify_PassengerIdType").html('<optgroup label="请选择证件类型">'+
	...
	function initSelects(){
		...
		var jq_studentEnterYear = jq("#modify_pStudentEnterYear optgroup");	
	...
	function refreshSucceeded(result){
		...
		if (mor.ticket.util.invocationIsSuccessful(invocationResult)) {
			...
			for (var i = 0, len = passengers.length; i < len; i++ ) {
				 p = passengers[i];
				 if (p.user_name === newP.user_name && p.id_no === newP.id_no) {
					 find = true;
				 }
			}
			}
