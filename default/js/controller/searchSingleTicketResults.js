	...
	jq("#searchSingleTicketResultsView")
			.live(
					...
					function() {
						...
						jq("#searchSingleResultsButtonGroup .ui-select .ui-btn-text span")
								.text("选择日期");
						if(query.purpose_codes==="0X"){
							jq("#searchSingleTicketResultsView .ui-header>h1").text("查询结果(学生)");
						}else if(query.purpose_codes==="1F"){
							jq("#searchSingleTicketResultsView .ui-header>h1").text("查询结果(农民工)");
						}else {
							jq("#searchSingleTicketResultsView .ui-header>h1").text("查询结果");
						}
	...
	function processYpInfo(ypinfo) {
		...
		for ( var i = 0, m = 6, n = 10, x = 0, y = 1; i < arrayLength; i++, m = m + 10, n = n + 10, x = x + 10, y = y + 10) {
			...
			if (parseInt(ypinfo.substring(m, m + 1), 10) == 3) {
				seat_type = "无座";
				seat_type_rate = 100;
				seat_num = parseInt(ypinfo.substring(m + 1, n), 10);
			} else {
...
//		}
	...
	function initChooseDateInputScroller() {
		...
		var findate = new Date(date.setDate(date.getDate() + reservePeriod - 1));
	...
	function initDateBtn() {
		...
		if (mor.ticket.viewControl.bookMode == "fc") {
			...
			} else if (date_back - nowDate > (reservePeriod - 2) * 86400000) {
			...
			if (date < nowDate) {
				jq("#prevDateBtn").addClass("ui-disabled");
				jq("#nextDateBtn").removeClass("ui-disabled");
			} else if (date - nowDate > (reservePeriod - 2) * 86400000) {
				jq("#nextDateBtn").addClass("ui-disabled");
				jq("#prevDateBtn").removeClass("ui-disabled");
			} else {
				jq("#prevDateBtn").removeClass("ui-disabled");
		...
		jq("#nextDateBtn")
				.bind(
						...
						function() {
							...
							var reservePeriod = parseInt(window.ticketStorage
									.getItem("reservePeriod"), 10);
							if (date - nowDate < (reservePeriod - 2) * 86400000) {
	...
	var ticketListTemplate = "{{~it :ticket:index}}"
			...
			+ "{{ for(var i=0;i<ticket.yplist.length;i++) { }}"
			+ "{{ if(ticket.yplist[i].num ){if(ticket.yplist[i].num <= 20) { }}"
			...
			+ "</div>"
			+ "{{ if(ticket.flag == '0'){ }}"
			+ "<div style='text-align: left;padding-left: 12px;'>{{=ticket.message}}</div>"
			+ "{{ } }}"
			+ "{{ if(ticket.yp_ex == '1'){ }}"
			+ "<div class='ticket-train-discount'>优惠</div>"
			+ "{{ } }}"
			+ "</a>" + "</li>" + "{{~}}";
