...
/* JavaScript content from js/controller/offLinePayFinish.js in folder common */
(function(){
   jq("#offLinePayFinishtView").live("pagebeforeshow", function(){
	   var user = mor.ticket.loginUser;
		if(user.isAuthenticated === "Y"){
			...
			if(payMode){
				...
				//jq("#offLinePayFinishtView .tips2 p").html(warmStr);
				jq("#offLinePayFinishMessage").html(dateStr);
				jq(".tipWarm").show();
			}else{
...
//	  WL.Logger.debug("###################################踩踩踩踩踩踩踩踩踩###:"+msg);
   ...
   function contentIscrollRefresh(){
		if(jq("#offLinePayFinishtView .ui-content").attr("data-iscroll")!=undefined){
			jq("#offLinePayFinishtView .ui-content").iscrollview("refresh");
		}
	}
