...
/* JavaScript content from js/controller/1farePassengers.js in folder common */
(function(){
...
//				});
			...
			jq(".car-head .car-head-number-box").html(ticket.station_train_code);
			
			jq(".car-head .car-head-starandover").html(cache.getStationNameByCode(ticket.from_station_telecode)+'<br /><span>'+ticket.start_time+'出发</span>');

			jq(".car-head .car-head-endandover").html(cache.getStationNameByCode(ticket.to_station_telecode)+'<br /><span>'+ticket.arrive_time+'入站</span>');
			
			//alert(ticket.lishiHour);
			var LishTime="";
			if (ticket.lishiHour!= '00'){
				LishTime+=ticket.lishiHour+"小时";
			}else{
				LishTime+=ticket.lishiMinute+"分钟";
			...
			+ "{{if(ticket.yplist[i].num <= 20) { }}"
			+ "<span class='{{=ticket.yplist[i].uclass}}'><span>{{=ticket.yplist[i].type}}:</span>{{=ticket.yplist[i].num}}</span>"
			+ "{{ } }}"
			+ "{{ else { }}"
			+ "<span class='{{=ticket.yplist[i].uclass}}'><span>{{=ticket.yplist[i].type}}: </span>有</span>"
			+ "{{ } }}"
			*/
	...
	function idTypeControl(){
		...
		function regsiterPassengersFunHandher(){
			...
			if (mor.ticket.farelist.length=='0' || mor.ticket.passenger.length!='0'){
				...
				mor.ticket.farelist.concat(mor.ticket.farelist,mor.ticket.passenger);
			...
			if (typeof mor.ticket.farelist !== "object"){
				mor.ticket.farelist.sort(mor.ticket.farelist);
			}
			
			for(var i =0;i<=mor.ticket.farelist.length-1;i++){
				
				temp+='<div class="car-peop-list" >';
				temp+='<div class="car-peop-list-name">'+mor.ticket.farelist[i][1]+'</div>';
				temp+='<div class="car-peop-list-numb">'+mor.ticket.farelist[i][0]+'</div>';
				temp+='<div class="car-peop-list-zx">'+ticket.yplist[0].type+'</div>';
				temp+='<div class="car-peop-list-pz">';
				temp+='<select class="changeSeatType" name="车票类型">"';
				temp+='"<optgroup label="请选择类型">';
				
				
				for(var ii in mor.ticket.classd){
				temp+='<option value="'+ii+'">'+mor.ticket.classd[ii]+'</option>';
				}
	...
	function TicketDeletePassengerBtnListeners(){
		jq(".car-peop-list-cz").live("tap", function(e){
				 ...
				 WL.SimpleDialog.show( 
					"温馨提示", "确定删除该乘客吗?",  
					 ...
					 {text: "确认", handler: function(){
						mor.ticket.farelist.splice(data_index, 1);
					    regsiterPassengersFunHandher();
						return;
					}}] 
				);
	...
	function TicketAddPassengerBtnListeners(){
		jq("#TicketAddPassengerBtn").off().bind("tap", function(){
	...
	function TicketEditSeatBtnListeners(){
		...
		jq("#editBtnSeat").live("tap", function(e){
			
			
			
			var data_val =jq(this).attr("data-val");
			
			if (data_val!=undefined){
				
				mor.ticket.leftTicketQuery.seat_id = data_val;
				regsiterYpListFunHandher();
			}
	...
	function PassengerFarelistListeners(){
		
		/*for(var ii in mor.ticket.passenger){
			
			
			
			  alert(mor.ticket.passenger[ii][0]);
			
		}*/
		/*var farelist = mor.ticket.Passenger.farelist;
		
			for(var i in mor.ticket.passenger){
	...
	function registerPassengersButtonsListener(){
		...
		jq("#addFarebt").bind("tap", function(){
			jq("#bookTicketView").css("pointer-events","none");
			jq.mobile.changePage("Farepassengers.html");
			return false;
		});	
	...
	function prepareChangeTicketParameters(){
		...
		//	alert();
			...
			var seatTypeArray=[];//用于储存席别数组
	    	for(var i=0; i<processedTicket.yplist.length; i++){
			     	WL.Logger.debug("pageNUM:" + ticket.yplist[i].num);
			
		
					if(ticket.yplist[i].num<=20){
						var TicketNum =ticket.yplist[i].num+"张";
						
					}else{
						var TicketNum ="有票";
					}
					
					BoxTemp+='<div class="car-box-five car-box-'
						+processedTicket.yplist.length
						+'"><a data-val="'
						+processedTicket.yplist[i].type_id
						+'"  class="editBtnSeat" '
						+GetCheckSeat(ticket.yplist[i].type_id,i)
						+'><span class="car-txt-zw">'
						+ticket.yplist[i].type
						+'</span><br /><span class="car-txt-num">'
						+TicketNum+'</span><br /><span class="car-txt-jg">￥'
						+ticket.yplist[i].price
						+'</span></a></div>';
		 ...
		 function GetCheckSeat(Name,Seat){
				  var seat_Type = mor.ticket.leftTicketQuery.seat_Type;
				   if (seat_Type=="" || Seat=="0"){
							return "class=\"hover\"";
				   }else if (Name == seat_Type){
							return "class=\"hover\"";
				   }else{
					   return; 
				   }
