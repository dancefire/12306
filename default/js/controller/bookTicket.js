...
(function() {
					...
					function(e, data) {
						...
						util.syncDef
								.then(function(serverCache) {
									var st = new Date(window.localStorage
											.getItem('serverTime'));
									var ct = new Date();
	...
	function refreshFormFromModel() {
		...
		if (!model.from_station_telecode) {
			...
			model.from_station_telecode = (window.ticketStorage
					.getItem("set_from_station_telecode") == null ? "VNP"
		...
		if (!model.to_station_telecode) {
			model.to_station_telecode = (window.ticketStorage
					.getItem("set_to_station_telecode") == null ? "TJP"
	...
	function initTimePeriodSelectScroller(type) {
		...
		var seartasdas = mor.ticket.reservePeriodList;
		var reservePeriod = parseInt(window.ticketStorage
				.getItem("reservePeriod_" + reservedPeriodType), 10);
		var jq_dateInput = jq("#trainDateInput optgroup");
		jq_dateInput.empty();
		var jq_dateInputBack = jq("#trainDateInputBack optgroup");
		jq_dateInputBack.empty();
		for ( var i = 0; i < reservePeriod+1; i++) {
			var date = new Date(window.localStorage.getItem('serverTime')
					|| mor.ticket.util.getNewDate());
	...
	function initSeatSelectScroller() {
		var reservePeriod = parseInt(window.ticketStorage
				.getItem("reservePeriod"), 10);
		var jq_dateInput = jq("#trainSeatInput optgroup");
		var jq_trainSeatInputBack = jq("#trainSeatInputBack optgroup");
		jq_dateInput.empty();
	...
	function registerDoubleTripClickHandler() {
		jq("#doubleTrip").bind("tap", function() {
			jq("#doubleTrip").addClass("ui-btn-active ui-state-persist");
			jq("#singleTrip").removeClass("ui-btn-active ui-state-persist");
			jq("#backInfo").show();
			...
			//by yiguo
			//jq("#bookTicketView .iscroll-wrapper").iscrollview('refresh');
			jq("#bookTicketView .iscroll-wrapper>.iscroll-scroller").height(600);
			setTimeout(function(){
	...
	function registerQuerySingleTicketBtnClickHandler() {
		jq("#querySingleTicketBtn").bind("tap",function(e) {
							e.stopImmediatePropagation();

							// 判断当前用户是否为激活用户，未激活用户不可以查询订单
	...
	function registerExitChangeTicketBtnHandler() {
		jq("#exitChangeTicket").off().bind("tap", function(e) {
					e.stopImmediatePropagation();
	...
	function registerAddFare2fareListBtnClickHandler() {
        jq("#FareHeadersBtnGroup").off()
            .delegate("a.add-btn","tap",function(){
            ...
            .delegate("a.ui-exit-btn","tap",function(e){
            	var node=jq(this);
                ...
                if (index) {
                    WL.SimpleDialog.show("温馨提示", "确定删除该乘客吗?", [ {
                        text : "取消",
                        handler : function() {
                    		node.removeClass('ui-btn-hover-c').removeClass('ui-btn-down-c').addClass('ui-btn-up-c');
                            return false;
                        }
                    }, {
                        text : "确认",
            ...
            .delegate("a.ui-exit-btn","taphold",function(){
            	var node=jq(this);
